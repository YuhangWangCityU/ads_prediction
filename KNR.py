...
# Read data
df = pd.read_excel('../Feature.xlsx', sheet_name= "ML_features")
features = df.iloc[:,:-1]
# features
target = df.iloc[:,-1]
# target
...
model_grid  = { 'n_neighbors' : [5,7,9,11,13,15],
...
plt.text(0.5, 0.3, 'KNR Test', horizontalalignment='left',
...
test1 = pd.DataFrame({'knr-rmse':RMSE_2nd})
test2 = pd.DataFrame({'knr-r2':R2_2nd})
test3 = pd.DataFrame({'knr-rmse_test':RMSE_2nd_test})
test4 = pd.DataFrame({'knr-r2_test':R2_2nd_test})   

# Creating Excel Writer Object from Pandas  
writer = pd.ExcelWriter('model_performance.xlsx', mode='a', engine='openpyxl')
workbook=writer.book
test1.to_excel(writer,sheet_name='knr1',startrow=0 , startcol=0)   
test2.to_excel(writer,sheet_name='knr2',startrow=0, startcol=0)
test3.to_excel(writer,sheet_name='knr3',startrow=0, startcol=0)
test4.to_excel(writer,sheet_name='knr4',startrow=0, startcol=0)
writer.close()  
